fetch('electronic.json')
    ...
    .then(data => {
        ...
        // Assuming we're only dealing with the "gaming" category for now
        const gaming = data.categories.find(category => category.name === 'Gaming Consoles');
        
        if (gaming && gaming.products) {
            gaming.products.forEach(product => {
                ...
                gamingContainer.appendChild(productLink);
            });
        } else {
            const noProductsMessage = document.createElement('p');
            noProductsMessage.textContent = 'No gaming found in the catalog.';
            gamingContainer.appendChild(noProductsMessage);
        }
    })
