fetch('electronic.json')
    ...
    .then(data => {
        ...
        // Assuming we're only dealing with the "drives" category for now
        const drives = data.categories.find(category => category.name === 'Storage Drives');
        
        if (drives && drives.products) {
            drives.products.forEach(product => {
                ...
                drivesContainer.appendChild(productLink);
            });
        } else {
            const noProductsMessage = document.createElement('p');
            noProductsMessage.textContent = 'No drives found in the catalog.';
            drivesContainer.appendChild(noProductsMessage);
        }
    })
