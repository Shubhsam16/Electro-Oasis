fetch('electronic.json')
    ...
    .then(data => {
        ...
        // Assuming we're only dealing with the "laptops" category for now
        const laptops = data.categories.find(category => category.name === 'Laptops');
        
        if (laptops && laptops.products) {
            laptops.products.forEach(product => {
                ...
                laptopsContainer.appendChild(productLink);
            });
        } else {
            const noProductsMessage = document.createElement('p');
            noProductsMessage.textContent = 'No laptops found in the catalog.';
            laptopsContainer.appendChild(noProductsMessage);
        }
    })
