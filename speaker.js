fetch('electronic.json')
    ...
    .then(data => {
        ...
        // Assuming we're only dealing with the "speakers" category for now
        const speakers = data.categories.find(category => category.name === 'Speakers');
        
        if (speakers && speakers.products) {
            speakers.products.forEach(product => {
                ...
                speakersContainer.appendChild(productLink);
            });
        } else {
            const noProductsMessage = document.createElement('p');
            noProductsMessage.textContent = 'No speakers found in the catalog.';
            speakersContainer.appendChild(noProductsMessage);
        }
    })
